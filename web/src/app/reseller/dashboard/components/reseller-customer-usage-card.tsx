...
import { fetchResellerCustomerUsage, type UserKeyUsageItem, type UserKeyUsageLog, type UserKeyUsageSummary } from "@/lib/api";

type ResellerCustomerUsageItem = UserKeyUsageItem & {
...
function RecentLogTable({ logs }: { logs: UserKeyUsageLog[] }) {
  if (logs.length === 0) {
    return <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">暂无调用日志。</div>;
  }
        ...
        <TableBody>
          {logs.map((log) => (
            <TableRow key={log.id}>
              ...
              <TableCell>
                <Badge variant={statusBadgeVariant(log.status)} className="rounded-md">{statusLabel(log.status)}</Badge>
              </TableCell>
...
export function ResellerCustomerUsageCard() {
  const didLoadRef = useRef(false);
  const [summary, setSummary] = useState<UserKeyUsageSummary>({
    total_calls: 0, success_calls: 0, failed_calls: 0, image_count: 0, active_users: 0,
  });
  const [items, setItems] = useState<ResellerCustomerUsageItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [imageTarget, setImageTarget] = useState<ResellerCustomerUsageItem | null>(null);
  const [logTarget, setLogTarget] = useState<ResellerCustomerUsageItem | null>(null);
  ...
  const loadUsage = async () => {
    setIsLoading(true);
    try {
      const data = await fetchResellerCustomerUsage();
    ...
    <>
        ...
        <CardContent className="space-y-6 p-6">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold tracking-tight">客户用量统计</h2>
                <p className="text-sm text-stone-500">按客户汇总调用、成功率、图片数量和最近记录。</p>
              </div>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            ...
            <MetricBlock label="活跃客户" value={summary.active_users} />
          </div>
                ...
                <TableBody>
                  {items.map((item) => (
                    <TableRow key={item.key_id}>
                      ...
                      <TableCell>{item.monthly_usage ?? "-"} / {item.monthly_limit === -1 ? "不限" : (item.monthly_limit ?? "-")}</TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Button variant="outline" className="h-8 rounded-lg border-stone-200 bg-white px-3 text-stone-600" onClick={() => setImageTarget(item)} disabled={item.recent_images.length === 0}>
                            <ImageIcon className="size-4" />
                            图片
                          </Button>
                          <Button variant="outline" className="h-8 rounded-lg border-stone-200 bg-white px-3 text-stone-600" onClick={() => setLogTarget(item)} disabled={item.recent_logs.length === 0}>
      ...
      <Dialog open={Boolean(imageTarget)} onOpenChange={(open) => (!open ? setImageTarget(null) : null)}>
        <DialogContent className="flex h-[min(88vh,820px)] w-[min(92vw,900px)] flex-col overflow-hidden rounded-2xl p-0">
          <DialogHeader className="shrink-0 border-b border-stone-100 px-6 py-5">
            <DialogTitle>{imageTarget?.key_name || "客户"} 的最近图片</DialogTitle>
          </DialogHeader>
          <div className="flex-1 overflow-y-auto p-6">
            {imageTarget && imageTarget.recent_images.length > 0 ? (
              ...
              <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">暂无图片记录。</div>
            )}
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={Boolean(logTarget)} onOpenChange={(open) => (!open ? setLogTarget(null) : null)}>
        <DialogContent className="flex h-[min(88vh,760px)] w-[min(94vw,980px)] flex-col overflow-hidden rounded-2xl p-0">
          <DialogHeader className="shrink-0 border-b border-stone-100 px-6 py-5">
            <DialogTitle>{logTarget?.key_name || "客户"} 的最近日志</DialogTitle>
          </DialogHeader>
          <div className="flex-1 overflow-y-auto p-6">
